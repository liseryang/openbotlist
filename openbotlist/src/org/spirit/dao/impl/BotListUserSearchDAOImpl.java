		...
		return getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(Session session) throws HibernateException {
						Query query = session.createQuery("from org.spirit.bean.impl.BotListEntityLinks link where (link.urlTitle like '%" +  searchQueryStr + "%' OR link.keywords like '%" + searchQueryStr + "%') order by link.createdOn desc");
